...
int main( int argc,const char** const argv ){
    ...
    bool rename_is_correct = boost::regex_match( argv_1, boost::basic_regex< char >( "s?([/|@#])(?:(?!\\1).)+\\1(?:(?!\\1).)*\\1(?:(?:gi?|ig)?(?:\\1-?[1-9]\\d?)?|i)?" ) );
    bool remove_is_correct = boost::regex_match( argv_1, boost::basic_regex< char >( "d?([/|@#])(?:(?!\\1).)+\\1(?:gi?|ig?)?" ) );
...
//    memset( remove_format, '\0', sizeof( remove_format ) );
    ...
    try{
        boost::basic_regex< char > regex ( match );
    } catch( const boost::regex_error& re ){
    ...
    boost::basic_regex< char >::flag_type regex_flag = boost::regex_constants::ECMAScript;
    for( const char chr : end_flag ){
        if( chr == 'i' )
            regex_flag = boost::regex_constants::icase;
    }
    boost::basic_regex< char > regex( match, regex_flag );

    boost::regex_constants::match_flag_type search_flag = boost::regex_constants::format_first_only;
    for( const char chr : end_flag ){
        if( chr == 'g' )
            search_flag = boost::regex_constants::match_default;
    }
    ...
    while( *clone_argv ){
        ...
        dir_name = *clone_argv;
            ...
            if( boost::regex_search( dir_name, match_result, regex ) ){

                old_name          = dir_name;
                old_name_colorize = boost::regex_replace( dir_name, regex, red_color_match , search_flag );
                ...
                if( index ){
                    ...
                    if( index < 0 ){    // for fixing the negative index
                        ...
                        while( boost::regex_search( dir_name, match_result, regex ) ){
                    ...
                    while( boost::regex_search( dir_name, match_result, regex ) ){
                        ...
                        if( ! --index ){
                            new_name_colorize += boost::regex_replace( dir_name, boost::basic_regex<char>( match ), green_color + substitute + reset_color, boost::regex_constants::format_first_only );
                            new_name          += boost::regex_replace( dir_name, boost::basic_regex<char>( match ), substitute, boost::regex_constants::format_first_only );
                    ...
                    new_name          = boost::regex_replace( dir_name, regex, substitute, search_flag );
                    new_name_colorize = boost::regex_replace( dir_name, regex, green_color + substitute + reset_color, search_flag );
                }
